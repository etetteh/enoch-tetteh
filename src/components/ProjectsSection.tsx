...
// Temporarily simplified highlight function
const highlightSkillsInDescriptionInternal = (
  description: string,
  _projectTechStack: string[], // unused for now
  uniquePrefix: string
): ReactNode[] => {
  if (typeof description !== 'string' || !description) {
    return [React.createElement(React.Fragment, { key: `${uniquePrefix}-empty` }, description || '')];
  }
  return [React.createElement(React.Fragment, { key: `${uniquePrefix}-desc` }, description)];
};
...
export function ProjectsSection() {
  ...
  // const carouselBlockRef = useRef<HTMLDivElement>(null); // Temporarily remove ref for fade-in

  const isTitleVisible = useFadeInOnScroll(titleRef);
  const isIntroTextVisible = useFadeInOnScroll(introTextRef, { delay: 100 });
  // const isCarouselBlockVisible = useFadeInOnScroll(carouselBlockRef, { delay: 200 }); // Temporarily disable
  ...
  // Ensure currentProject is always valid
  ...
  if (!currentProject) {
    ...
    return <div>Error loading project data.</div>; // Fallback UI
  }
      ...
      <div className="container mx-auto">
        ...
        <p
          ...
          className={cn(
            ...
            "transition-opacity duration-1000 ease-out",
            isIntroTextVisible ? "opacity-100 delay-100" : "opacity-0"
          )}
        >
        ...
        <div
          // ref={carouselBlockRef} // Temporarily removed ref
          className={cn(
            "my-8",
            "opacity-100 scale-100" // Forced visibility
          )}
        >
          <div
            ref={carouselRef}
          ...
          >
            <div className={cn(
              ...
              <div
                ref={textContentRef}
                key={currentProject.id + '-text-pane'} // Re-trigger animation on slide change
                className="w-full md:w-1/2 md:h-full flex flex-col" // Removed animation classes
              >
                 <ScrollArea className="flex-grow min-h-0">
                    <div className="p-1 md:p-2 lg:p-4 space-y-3 text-center md:text-left">
                      ...
                      {currentProject.keyAchievement && (
                        <p className="text-xs sm:text-sm md:text-md font-semibold text-foreground mt-1 mb-2">
                          {currentProject.keyAchievement}
                        </p>
                      )}
                      ...
                      <div className="pt-3"> {/* Removed animation classes */}
                ...
                <div className="flex-shrink-0 flex gap-2 justify-center md:justify-start pt-4 border-t border-border/30 mt-auto p-1 md:p-2 lg:p-4"> {/* Removed animation classes */}
                  {currentProject.githubUrl && (
                    <Button variant="outline" size="sm" asChild className="transform transition-transform hover:scale-105">
                      <a href={currentProject.githubUrl} target="_blank" rel="noopener noreferrer">
                        <Github /> GitHub
              ...
              >
                <div className="text-center"> {/* Removed animation classes */}
